...
function googleSearch(query:string){return `https://www.google.com/search?q=${encodeURIComponent(`site:linkedin.com/in ${query}`)}`}
...
export default async function ContactsPage({searchParams}:{searchParams:Promise<{job?:string}>}){const {job:id}=await searchParams;const job=id?await getJob(id):null;
  ...
  const contacts=await listContactsForJob(job.id);const searches=[
    {label:"Recruiting partner",why:"May own the hiring process and clarify timing.",query:`${job.company} recruiter talent acquisition ${job.roleFamily}`},
    {label:"Likely hiring manager",why:"Look for the function leader one level above this role.",query:`${job.company} head director ${job.roleFamily}`},
    {label:"Potential peer",why:"Can share practical team and role context.",query:`${job.company} ${job.title}`},
  ];
