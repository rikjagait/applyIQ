...
export default async function JobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="content"><PageHead eyebrow={`${job.company} · ${job.location}`} title={job.title} copy={`${job.arrangement} · ${job.salary ?? "Salary not listed"} · Posted ${job.postedDaysAgo} days ago`} action={<div className="actions"><Link className="btn" href="/jobs">Back to jobs</Link><StartApplicationButton jobId={job.id}/></div>}/><section className="card top-application-actions"><div><div className="eyebrow">Choose your next step</div><h2>Application actions</h2><p className="subtle">Prepare the application, find someone at the company, or tell ApplyIQ this role is not right.</p></div><div className="top-application-buttons"><Link className="btn primary" href={`/jobs/${job.id}/studio`}>Open Application Studio</Link><Link className="btn" href={`/contacts?job=${job.id}`}>Find relevant contacts</Link><JobFeedback jobId={job.id}/></div></section><div className="detail-grid"><div className="stack">
