...
export default async function JobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="content"><PageHead eyebrow={`${job.company} · ${job.location}`} title={job.title} copy={`${job.arrangement} · ${job.salary ?? "Salary not listed"} · Posted ${job.postedDaysAgo} days ago`} action={<Link className="btn" href="/jobs">Back to jobs</Link>}/><section className="card next-step-card"><div><div className="eyebrow">Your recommended next step</div><h2>Prepare this application</h2><p className="subtle">Tailor the résumé, prepare answers and create the application pack.</p></div><div className="next-step-actions"><StartApplicationButton jobId={job.id}/><Link className="text-action" href={`/contacts?job=${job.id}`}>Or find a relevant contact</Link></div><details className="more-actions"><summary>More actions</summary><div className="actions"><ReanalyzeJobButton jobId={job.id}/><JobFeedback jobId={job.id}/></div></details></section><div className="detail-grid"><div className="stack">
