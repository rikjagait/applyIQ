...
async function GeneratedStudio({job}:{job:Job}){const studio=await getApplicationStudio(job);return <>
  ...
  <section className="section card"><div className="section-head"><div><div className="eyebrow">Step 4 · Outreach</div><h2>Find the right people and prepare a personal message</h2><p className="subtle">ApplyIQ researches relevant public professional profiles and drafts grounded messages. Neelam reviews and sends everything manually.</p></div><a className="btn primary" href={`/contacts?job=${job.id}`}>Open outreach workspace <ArrowUpRight size={14}/></a></div></section>
...
export default async function StudioPage({params}:{params:Promise<{id:string}>}) {
  ...
  return <div className="content">
    <PageHead eyebrow="Application Studio" title={`${job.title} · ${job.company}`} copy="Your application workspace is open. AI tailoring can continue loading below without holding up the page." action={<div className="actions">{job.sourceUrl?<a className="btn" href={job.sourceUrl} target="_blank" rel="noreferrer">View job posting <ArrowUpRight size={14}/></a>:null}<a className="btn primary" href={`/api/applications/studio/${job.id}/export`}><Download size={14}/>Download full pack</a></div>}/>
