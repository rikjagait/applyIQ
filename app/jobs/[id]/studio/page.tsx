...
export default async function StudioPage({params}:{params:Promise<{id:string}>}) {
  const {id}=await params; const job=await getJob(id); if(!job)notFound(); const studio=buildApplicationStudio(job);
  return <div className="content">
    <PageHead eyebrow="Application Studio" title={`${job.title} · ${job.company}`} copy="Review your tailored résumé, prepare the application and continue to the employer in one guided flow." action={<div className="actions">{job.sourceUrl?<a className="btn" href={job.sourceUrl} target="_blank" rel="noreferrer">View job posting <ArrowUpRight size={14}/></a>:null}<a className="btn primary" href={`/api/applications/studio/${job.id}/export`}><Download size={14}/>Download full pack</a></div>}/>
