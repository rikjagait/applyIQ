...
export async function POST(request:Request){
  try{
    ...
    const discovered=boardUrl?{jobs:await discoverAtsJobs(boardUrl),boardUrl,directoryUpdatedAt:null,source:"Direct careers URL"}:await discoverAtsJobsByCompanyName(companyName);
    const preferences=await getJobPreferences();const allJobs=discovered.jobs.map(job=>({...job,discoveredVia:job.discoveredVia??"Direct employer feed" as const}));const jobs=prioritizeDiscoveredJobs(allJobs,50,preferences);
    jobs.sort((a,b)=>(b.postedAt?Date.parse(b.postedAt):0)-(a.postedAt?Date.parse(a.postedAt):0));
    return Response.json({jobs,scanned:allJobs.length,boardUrl:discovered.boardUrl,source:discovered.source,directoryUpdatedAt:discovered.directoryUpdatedAt});
