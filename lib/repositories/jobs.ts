...
function mapRow(row: JobRow): Job {
  ...
  return {id:row.id,title:row.title,company:company?.name??"Unknown company",location:row.location??"Location not specified",arrangement:(row.arrangement==="remote"?"Remote":row.arrangement==="hybrid"?"Hybrid":"On-site") as WorkArrangement,salary,employmentType:row.employment_type==="Part-time"?"Part-time":"Full-time",roleFamily:row.role_family??"Adjacent Opportunity",industry:company?.industry??"Not specified",postedDaysAgo:row.date_posted?Math.max(0,Math.floor((Date.now()-new Date(row.date_posted).getTime())/86400000)):Math.max(0,Math.floor((Date.now()-new Date(row.created_at).getTime())/86400000)),source:"Saved URL",sourceUrl:source?.original_url||source?.source_url||undefined,score,probability:Number(match?.interview_probability??0),category,summary:match?.explanation??"Analysis pending.",strengths:textArray(match?.strengths),gaps:textArray(match?.gaps),requirements:(row.job_requirements??[]).map(r=>({requirement:r.requirement,importance:r.importance==="preferred"?"Preferred":"Required",category:r.category??undefined,sourceQuote:r.source_quote??undefined,evidence:r.evidence??"Re-screen this role to map résumé evidence",strength:(r.strength==="Strong"||r.strength==="None"?r.strength:"Moderate") as "Strong"|"Moderate"|"None",gap:r.gap??"Re-screen this role using the latest résumé"})),status:statusMap[row.applications?.[0]?.status]??"Discovered"};
}
