...
function slugFromUrl(value: string) {
  ...
  if (
    ...
    return { provider: "greenhouse" as const, slug: parts[0] };
  if (url.hostname === "jobs.lever.co" || url.hostname === "api.lever.co")
    return { provider: "lever" as const, slug: parts.at(-1) };
  if (url.hostname === "jobs.ashbyhq.com")
    return { provider: "ashby" as const, slug: parts[0] };
  if (url.hostname === "careers.smartrecruiters.com")
    return { provider: "smartrecruiters" as const, slug: parts[0] };
  if (url.hostname.endsWith(".myworkdayjobs.com"))
    return {
      provider: "workday" as const,
      slug: url.hostname.split(".")[0],
      instance: url.hostname.split(".").slice(1, -2).join("."),
      site: parts[0],
    };
  throw new Error(
    "Use a Greenhouse, Lever, Ashby, SmartRecruiters, or Workday company careers URL.",
  );
}
...
function salaryFromText(value?: string) {
  ...
  return text.match(
    /(?:US\$|\$)\s?\d{2,3}(?:,\d{3})?(?:\s*(?:-|–|to)\s*(?:US\$|\$)?\s?\d{2,3}(?:,\d{3})?)?(?:\s*(?:per year|annually|\/year|\/yr))?/i,
  )?.[0];
}
...
async function getJson(url: string) {
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "user-agent": "ApplyIQ/1.0 job discovery",
    },
    cache: "no-store",
  });
  if (!response.ok)
    throw new Error(`The careers board returned ${response.status}.`);
  return response.json() as Promise<unknown>;
}

export async function discoverAtsJobs(
  boardUrl: string,
): Promise<DiscoveredJob[]> {
  const board = slugFromUrl(boardUrl);
  if (!board.slug)
    throw new Error("The careers board URL is missing its company name.");
  if (board.provider === "greenhouse") {
    ...
    return (data.jobs ?? []).map((job) => ({
      ...
      provider: "Greenhouse",
      description: plain(job.content),
      salary: salaryFromText(job.content),
      department: job.departments?.map((x) => x.name).join(", "),
    }));
  }
  if (board.provider === "lever") {
    ...
    return data.map((job) => ({
      ...
      salary: salaryFromText(job.descriptionPlain),
  ...
  if (board.provider === "ashby") {
    ...
    return (data.jobs ?? []).map((job) => ({
      ...
      salary:
        ...
        salaryFromText(job.descriptionPlain),
      department: job.department,
      employmentType: job.employmentType,
      arrangement: job.isRemote ? "Remote" : undefined,
    }));
  }
  if (board.provider === "smartrecruiters") {
    const data = (await getJson(
      `https://api.smartrecruiters.com/v1/companies/${encodeURIComponent(board.slug)}/postings?limit=100&offset=0`,
    )) as {
      content?: Array<{
        id: string;
        name: string;
...
function companySlugs(name: string) {
  const base = name
    .toLowerCase()
    .replace(/\b(inc|llc|ltd|limited|company|co|group|holdings)\b\.?/g, "")
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
  return [
    ...new Set(
...
export async function discoverAtsJobsByCompanyName(companyName: string) {
  try {
    ...
    const supported = directory.matches.filter((match) =>
      ["greenhouse", "lever", "ashby", "smartrecruiters", "workday"].includes(
        match.ats.toLowerCase(),
      ),
    );
        ...
        if (jobs.length)
          return {
            jobs: jobs.map((job) => ({
              ...
              discoveredVia: "Stapply" as const,
            })),
            boardUrl: company.url,
            directoryUpdatedAt: directory.updatedAt,
            source: "Stapply open jobs directory",
          };
      } catch {
        /* Try the next exact directory match. */
      }
    }
  } catch {
    /* Fall back to direct ATS slug detection when the directory is unavailable. */
  }
  ...
  const attempts = await Promise.all(
    candidates.map(async (boardUrl) => {
      try {
        const jobs = await discoverAtsJobs(boardUrl);
        return jobs.length ? { jobs, boardUrl } : null;
      } catch {
        return null;
      }
    }),
  ...
  if (!match)
    throw new Error(
      "ApplyIQ could not automatically locate a supported careers board for that company. Open ‘Advanced’ and paste its careers URL instead.",
    );
  return {
    ...match,
    jobs: match.jobs.map((job) => ({
      ...job,
      discoveredVia: "Direct employer feed" as const,
    })),
    directoryUpdatedAt: null,
    source: "Direct ATS lookup",
  };
}
