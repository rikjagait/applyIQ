...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <><AuthInviteBridge/><div className="shell">
    <aside className="sidebar">
      ...
      <div className="user"><span className="avatar">NJ</span><div><strong style={{fontSize: 13}}>Neelam Jagait</strong><small>Private workspace</small></div></div>
    </aside>
    <main className="main"><header className="topbar"><div className="history-nav" aria-label="Page history"><button className="icon-btn" type="button" onClick={()=>router.back()} aria-label="Go back" title="Back"><ArrowLeft size={16}/></button><button className="icon-btn" type="button" onClick={()=>window.history.forward()} aria-label="Go forward" title="Forward"><ArrowRight size={16}/></button></div><label className="quick-nav"><span>Go to</span><select value={currentSection} onChange={(event)=>router.push(event.target.value)} aria-label="Navigate to a workspace section"><optgroup label="Your workflow">{primaryLinks.map(([label, href])=><option key={href} value={href}>{label}</option>)}</optgroup><optgroup label="More tools">{toolLinks.map(([label, href])=><option key={href} value={href}>{label}</option>)}</optgroup></select></label><div className="topbar-account"><span className={connected?"pill workspace-status":"demo"}><i/>{connected?"Live workspace":"Demo mode"}</span>{connected&&<form action="/auth/logout" method="post"><button className="btn quiet-btn" type="submit">Sign out</button></form>}</div></header>{children}</main>
  </div></>;
}
